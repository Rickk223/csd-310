...
# main methid containing the menu method
def main():
   menu()

# method to display main menu options and handle user choices
def menu():
...
def view_store_locations():
    ...
    for location in locations:
        print("  Locale: {}\n".format(location[1]))
        break
    # keep this identention for menu to display properly
    menu()
...
def my_account():
    ...
    while True:
        try:
            # user_id will appear as undeclared since we are waiting for user's input
            global user_id
            user_id = (int(input("""
...
# diplay user's menu once user's identity is verified
    ...
    while True:
        try:
            ...
            if account_choice == 1 :
                ...
                menu()
                break
            # exit user account and go back to main menu
